class AuthorizationCodeTest < MiniTest::Unit::TestCase
  def setup
    ...
    @config = mock()
    @config.stubs(:user_datastore).returns(mock())
    @config.stubs(:client_datastore).returns(mock())
    @config.stubs(:code_datastore).returns(mock())
    @config.stubs(:token_datastore).returns(mock())
  ...
  def test_should_raise_invalid_client
    ...
    request_handler = OAuth2::Provider::Strategy::AuthorizationCode.new(request)
  ...
  def test_should_return_authorization_code
    ...
    strategy = OAuth2::Provider::Strategy::AuthorizationCode.new(request)
